...
if IV is not None:
    try:
        ...
        for d in IV.CALENDARIO_PARADAS:
            ...
            _PARADAS_DEFAULT.append({"dia": d, "label": dt.strftime("%d %b %Y")})
    except Exception:
        pass
...
def _num(id_, val, step=1, min_=None, max_=None):
    kw = {}
    if min_ is not None: kw["min"] = min_
    if max_ is not None: kw["max"] = max_
    return dbc.Input(id=id_, type="number", value=val, step=step,
...
def _panel_mod1():
    ...
    return dbc.Card([
        ...
        dbc.CardBody([
            ...
            dbc.ListGroup(
                id="lista-paradas",
                children=[_parada_row(p["dia"], p["label"], i) for i, p in enumerate(_PARADAS_DEFAULT)],
                flush=True, className="small",
            ),
            dcc.Store(id="store-paradas", data=_PARADAS_DEFAULT),
        ]),
...
def guardar_todo(_, acciones, valor_ac, desm_ini, desm_fin, frec_turno, dur_mant, paradas,
                 fecha_ini, particiones, presupuesto, cc, pmp, alpha, stock_sub, dias_sub,
                 reg_sel, reg_nombre, reg_hectareas, reg_frec, reg_cap_est, reg_nivel_est,
                 tiene_estanque, reg_id):
    errores = []
    try:
    ...
    try:
        _patch_py_file(PAR_PATH, {
            ...
            "STOCK_SUBTERRANEO_INICIAL_M3":    float(stock_sub) if stock_sub is not None else 0.0,
            "DIAS_SIN_RIEGO_PARA_SUBTERRANEA": int(dias_sub) if dias_sub is not None else 0,
...
def simular_todo(_):
    ...
    bat_lines = [
        ...
        "echo ============================================================",
        "echo.",
        # os.startfile abre exactamente una vez en el programa predeterminado para .html
        f'if exist "{reporte_partic}" ("{python}" -c "import os; os.startfile(r\'{reporte_partic}\')")',
        "pause",
    ]
